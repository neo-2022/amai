...
- `./scripts/proof_memory_external_real_bounded.sh` proves bounded, dataset-specific execution evidence for LongMemEval only: real normalized `longmemeval_s_cleaned` requests, real Amai runtime predictions, runtime metrics, baseline score output, and a repo-owned local semantic retrieval-evidence summary for `AMAI_EXTERNAL_MEMORY_REAL_LIMIT` cases.
- `./scripts/proof_memory_external_real_bounded_ama_bench.sh` proves bounded, dataset-specific execution evidence for AMA-Bench only: real normalized `ama_bench_manual` requests, real Amai runtime predictions, runtime metrics, baseline score output, and a repo-owned local semantic retrieval-evidence summary for `AMAI_EXTERNAL_MEMORY_REAL_LIMIT` cases while keeping official scorer contract unavailable/blocker-visible.
- `./scripts/proof_memory_external_real_bounded_memoryagentbench.sh` proves bounded, dataset-specific execution evidence for MemoryAgentBench only: real normalized `memoryagentbench_conflict_resolution`, `memoryagentbench_long_range_understanding`, and `memoryagentbench_test_time_learning` requests, real Amai runtime predictions, runtime metrics, answer-source accounting, baseline score output, a repo-owned local semantic retrieval-evidence summary, and explicit boolean-typed `runtime_corpus_sha256` / `runtime_corpus_reused_from_previous_case` accounting for `AMAI_EXTERNAL_MEMORY_REAL_LIMIT` cases while keeping official scorer contract unavailable/blocker-visible.
- `./scripts/proof_memory_external_real_bounded_memoryagentbench_accurate_retrieval_blocked.sh` proves a different bounded outcome for `memoryagentbench_accurate_retrieval`: on the current default bounded limit (`AMAI_EXTERNAL_MEMORY_REAL_LIMIT=3`), the slice now runs end-to-end with perfect bounded baseline answers and no benchmark-specific shaping in the bounded run (`query_override_cases=0`, `window_override_cases=0`, `answer_extraction_cases=0`, `generic_runtime_maturity=true`), but the primary repo-owned local semantic retrieval judge remains negative on the current ranked preview set (`question_relevant_cases=0/3`, `gold_answer_supported_cases=0/3`). Runtime proxy/accounting is therefore kept only as secondary blocker-visible evidence on this slice: `retrieval_answer_cases=2`, `fallback_scan_cases=1`, `top_ranked_relevant_retrieval_cases=2`, `gold_answer_supported_retrieval_cases=0`, and `top_ranked_structural_fact_supported_cases=0`. The runtime also now proves identical-corpus reuse inside that default bounded slice: one corpus hash across all three cases, `runtime_corpus_reused_cases=2`, and `index_project_ms=0` on the reused cases. A fresh profiling-guided runtime fix then corrected the synthetic-runtime edge-cache skip gate to match the actual `.md` runtime corpus shape instead of a stale `source_kind` assumption; in the current bounded run this drops cold first-case `index_project_ms` to about `0.87s`, bounded `index_project_ms.avg` to about `0.29s`, and bounded `total_case_ms.avg` to about `0.91s` while preserving the same bounded proof contract. This reuse is intentionally narrow: same-process, same-run, byte-identical materialized corpus reuse only, with a fail-closed guard on the currently materialized `paths.txt`; it is not a persistent cross-run cache, not semantic equivalence, and not a claim about hidden environment identity outside the materialized runtime corpus. It still must stay blocker-visible instead of being promoted into the fully trusted bounded-runtime whitelist because the primary local semantic lane is still negative, benchmark-grade scorer parity is still absent, and `latency_maturity=false` remains a bounded-only disclaimer rather than a cold-index-dominance blocker. The boundary accounting is carried by explicit runtime metric flags rather than question-text re-derivation. The proof output also writes `bounded-proof-contract.json`; use it as the machine-readable disclaimer for the bounded blocked scope, the non-semantic interpretation of retrieval-hit rates, the negative local semantic verdict on the current ranked-preview-set lane, and the still-non-grade latency profile. The script now also re-runs targeted Rust negative tests for malformed resume identity fields and for the `paths.txt` same-hash reuse gate, so the reject-path story is not happy-path-only; those tests prove reject-path behavior, not a broader filesystem-stability guarantee.
...
- Official LongMemEval judge provenance is now fail-closed on endpoint identity too: the default official lane requires `https://api.openai.com/v1`. Any other OpenAI-compatible relay/proxy endpoint must block the official lane by default with `official_judge_api_base_url_not_official`; only the explicit local proof/debug override may use a non-official base URL, and those artifacts must stay non-official provenance.
- `./scripts/proof_memory_external_provider_diagnostic.sh` is the provider-path diagnostic lane for this gap: it probes `/models`, `/responses`, and `/chat/completions` on a configured OpenAI-compatible base URL, proves whether the provider is live for the current client model, and separately shows whether the official LongMemEval judge model/path is actually available there. This is diagnostic truth only; it must not be cited as official judge execution or upstream parity evidence.
- `./scripts/proof_memory_external_local_gemma_judge_live_balanced.sh` is the provider-independent local judge lane: it selects one raw `longmemeval_s_cleaned` record for each official LongMemEval question type, normalizes through `external-memory-prepare --source-path`, runs six Amai predictions, judges them through local Ollama chat (default `gemma4:e4b`), and then reconciles that local eval log through `external-memory-local-score`. This lane is intentionally non-official: it must keep `official_upstream_provenance_eligible=false` and `official_upstream_scorer_parity=false`, and it may be cited only for local regression/diagnostic evidence.
- `./scripts/proof_memory_external_local_semantic_retrieval_judge.sh` is the bounded repo-owned semantic retrieval lane: it consumes `cases.jsonl` plus runtime `*.case-metrics.jsonl`, judges the ranked preview set through local Ollama (default `gemma4:e4b`), writes per-case JSONL plus `external_memory_local_semantic_retrieval_judge_v2`, and is the primary bounded retrieval-relevance/support proof layer for the `proof_memory_external_real_bounded*.sh` scripts. It also fail-closes on empty cases/case-metrics, blank base URL or model, and case-metric bench/dataset mismatches before any live judge attempt, while malformed ranked preview rows surface as `case_blocker_counts` instead of being silently accepted. It is intentionally non-official and ranked-preview-set-scoped.
...
- Fresh bounded MemoryAgentBench runtime note 2026-04-27: `./scripts/proof_memory_external_real_bounded_memoryagentbench.sh` now materializes three small MemoryAgentBench slices with real runtime predictions, baseline score output and runtime metrics boundaries: `memoryagentbench_conflict_resolution`, `memoryagentbench_long_range_understanding`, and `memoryagentbench_test_time_learning`. This remains `bounded_real_runtime_score` only: `memoryagentbench_overall_score` is baseline-only, retrieval relevance is still proxy/lexical on the runtime metrics, and official scorer parity remains unavailable. The verified profiles are operationally different: `conflict_resolution` showed fallback-scan-only answers on the current bounded slice, `long_range_understanding` ran with much larger contexts and higher case latency, and `test_time_learning` ran with very large context packs and high latency but does not imply general adaptive learning maturity beyond the bounded slice. Fresh reruns also materialize the repo-owned local semantic retrieval judge summary for these named profiles; on `conflict_resolution` that semantic lane is currently negative (`question_relevant_cases=0/3`, `gold_answer_supported_cases=0/3`) even though runtime retrieval hits are present. This is a bounded proof contract for those datasets, not a general promotion gate.
...
- Fresh bounded MemoryAgentBench `accurate_retrieval` note 2026-04-27: a bounded `3-case` slice now materializes runtime, perfect bounded baseline score output, and a clean generic-runtime shaping boundary, but it does not materialize positive semantic retrieval support on the current ranked-preview-set lane. In the current bounded run, `memoryagentbench_overall_score=1.0`, `exact_match=3/3`, `retrieval_answer_cases=2`, `fallback_scan_cases=1`, `relevant_retrieval_evidence_cases=2`, `gold_answer_supported_retrieval_cases=0`, `top_ranked_structural_fact_supported_cases=0`, `question_relevant_cases=0/3`, `gold_answer_supported_cases=0/3`, and `benchmark_specific_shaping_boundary` is now clean with `query_override_cases=0`, `window_override_cases=0`, `answer_extraction_cases=0`, `benchmark_specific_shaping_present=false`, `generic_runtime_maturity=true`. Runtime resume is now fail-closed on artifact identity: `requests.jsonl` must carry non-empty `bench` and `dataset`, persisted `.case-metrics.jsonl` rows must carry the explicit shaping flags, and the newer top-ranked retrieval telemetry keys must also be present or the runtime refuses to reuse them. This means the slice is now stronger bounded generic-runtime evidence than before, but the contour is still not fully trusted benchmark-grade maturity because the primary local semantic lane is negative and upstream scorer parity is still absent.
...
- `predictions.jsonl.metrics.json.answer_source_boundary.boundary_version` is currently `external_memory_answer_source_boundary_v1`; non-zero `retrieval_hit_cases` are required for the bounded proof, but `retrieval_answer_cases > 0` is not a universal requirement because some bounded slices truthfully complete through fallback scan while still materializing retrieval hits and local semantic evidence. Any `fallback_scan_cases > 0` remains evidence that full semantic retrieval precision is not proven.
...
- For the bounded `memoryagentbench_accurate_retrieval` proof, the current truthful contract is blocker-visible rather than `top_ranked_relevant_retrieval_rate == 1.0`: the same fixed `AMAI_EXTERNAL_MEMORY_REAL_LIMIT=3` slice now proves a negative local semantic verdict on the ranked preview set, and the corresponding `bounded-proof-contract.json` must keep that negative semantic result plus the proxy failure modes surfaced instead of flattening them into a positive threshold claim.
...
- `proof_memory_external_provider_diagnostic.sh` may be cited only as provider compatibility evidence. It can prove that a relay is live for the current client model on `/responses` while still showing that the same relay does not expose the official judge model or does not serve `/chat/completions` reliably. That diagnostic does not weaken the official parity law and does not convert a non-official model/path into LongMemEval scorer evidence.
...
- `external-memory-local-judge.boundary_version` is currently `external_memory_local_judge_execution_v1`; it reuses the embedded upstream LongMemEval prompt templates but executes them through local Ollama chat, writes `local_judge_provenance`, and must always keep `official_upstream_provenance_eligible=false` and `official_upstream_scorer_parity=false`.
- `external-memory-local-score.boundary_version` is currently `external_memory_local_score_reconciliation_v1`; a reconciled output requires all six official LongMemEval question types, matching `question_id` values, valid `autoeval_label` records and, when requested, a matching local model label. `metrics.abstention_accuracy` may be `null` when the curated proof slice has `abstention_count=0`; that is a truthful local-slice result, not a contract failure.

Proxy relevance limitations:
- The query-overlap proxy is expected to catch only direct lexical overlap between the benchmark question and retrieved snippets; it is intentionally conservative evidence that retrieval participated, not proof that the snippet semantically answers the question. The bounded proof lane now uses the local semantic retrieval judge as primary evidence and keeps this proxy only as secondary accounting.
...
- The gold-answer boundary is fail-closed: supported cases cannot exceed non-empty retrieval evidence cases, and all maturity fields remain blocked until an official upstream scorer or benchmark-grade semantic judge is wired in. The bounded proof lane now prefers the local semantic retrieval judge summary over this lexical overlap proxy when deciding whether top-ranked retrieval evidence is answer-supporting.
...
- bounded local semantic retrieval proof surface: `./scripts/proof_memory_external_local_semantic_retrieval_judge.sh`;
...
- balanced local Gemma judge proof surface: `./scripts/proof_memory_external_local_gemma_judge_live_balanced.sh`;
...
- accepted freshness: 2026-04-25 bounded prep rerun, 2026-04-26 bounded real LongMemEval runtime+baseline-score proof, plus 2026-06-01 local Ollama/Gemma balanced judge proof;
