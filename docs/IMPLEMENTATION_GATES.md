...
- Runtime metrics must include `answer_source_boundary.boundary_version = external_memory_answer_source_boundary_v1`; bounded proof must require non-zero `retrieval_hit_cases`, but `retrieval_answer_cases > 0` is no longer a universal closure contract because some bounded slices truthfully complete through fallback scan while still materializing retrieval hits and local semantic evidence. `semantic_precision_maturity` stays `false` until retrieval-answer coverage and semantic relevance are separately validated.
- Runtime metrics must still include `retrieval_relevance_boundary.boundary_version = external_memory_retrieval_relevance_boundary_v1` and `gold_answer_relevance_boundary.boundary_version = external_memory_gold_answer_relevance_boundary_v1`, but bounded proof closure must no longer depend on their positive proxy counts as the primary evidence layer. These two boundaries remain legacy accounting only: `judge_kind=query_overlap_proxy` and `judge_kind=gold_answer_lexical_overlap` must stay visible as blocker-bearing secondary signals, not be silently promoted into semantic precision.
- Every bounded real external-memory proof must also run `./scripts/proof_memory_external_local_semantic_retrieval_judge.sh` and validate its summary `boundary_version = external_memory_local_semantic_retrieval_judge_v2`. That local lane is the primary bounded semantic-evidence contour: it judges the ranked preview set against the benchmark question and, when a gold answer exists, against the benchmark answer through a local Ollama model. The underlying lane now also validates the ranked-preview contract itself: empty cases, empty case-metrics, blank base URL or model, and case-metric bench/dataset mismatches fail closed before the live judge loop, while malformed ranked preview rows are surfaced as case-level blockers in `case_blocker_counts` instead of being silently treated as clean evidence. It must keep `official_upstream_provenance_eligible=false`, `official_upstream_scorer_parity=false`, `semantic_precision_maturity=false`, and blocker `ranked_retrieval_preview_set_only` visible because it is still a bounded local diagnostic lane, not full retrieval-distribution maturity or upstream parity.
...
- `./scripts/proof_memory_external_provider_diagnostic.sh` is the provider-path diagnostic guard for this gap: it must enumerate the provider catalog through `/models`, probe the current client model through `/responses`, and separately probe `/chat/completions` for both the provider model and the official LongMemEval judge model. It may prove `provider live` and `official judge incompatible` at the same time; that combination is diagnostic truth, not official scorer evidence.
- `./scripts/proof_memory_external_local_gemma_judge_live_balanced.sh` is the provider-independent local regression guard: it must select one raw `longmemeval_s_cleaned` record for each official question type, normalize through `external-memory-prepare --source-path`, run six Amai runtime predictions, execute `external-memory-local-judge` against a local Ollama model, and reconcile that log with `external-memory-local-score`. The summary and score artifacts must keep `official_upstream_provenance_eligible=false` and `official_upstream_scorer_parity=false`. This contour is valid for local diagnostics/regression only; it must not be cited as official parity evidence.
- `./scripts/proof_memory_external_local_semantic_retrieval_judge.sh` is the bounded repo-owned semantic retrieval-evidence guard. It consumes normalized `cases.jsonl` plus runtime `*.case-metrics.jsonl`, judges the ranked preview set through local Ollama, writes per-case `judge_results` JSONL and a machine-readable summary, and is the required primary evidence layer for bounded retrieval relevance/support claims in `proof_memory_external_real_bounded*.sh`. It now fail-closes on empty cases/case-metrics, blank base URL or model, and case-metric bench/dataset mismatches before any live judge call, and it surfaces malformed ranked preview rows as `case_blocker_counts` rather than silently accepting them. It is intentionally non-official and ranked-preview-set-scoped; it must not be cited as full retrieval-distribution maturity or upstream scorer parity.
- `external-memory-run` itself must not materialize a nested synthetic project from `state/external-benchmarks/runtime/**/repo`. Runtime corpus indexing for benchmark cases must stay inside the already bound caller project plus the requested namespace; creating a descendant alias project there is a fail-closed defect against the repo-root binding law.
...
- balanced local Gemma judge proof = provider-independent local runtime/judge/score regression evidence only; it proves the local non-official lane and must keep `official_upstream_scorer_parity=false`;
- synthetic `external-memory-official-score` eval-log reconciliation = official log schema/metric contract only;
- bounded real `external-memory-run/score` proof = named dataset/limit runtime+baseline-score evidence plus answer-source accounting, with local semantic retrieval evidence judged through `external_memory_local_semantic_retrieval_judge_v2`; this is still bounded/local evidence, not full retrieval precision or benchmark maturity;
...
- `AMAI-AUDIT-EXTMEM-005`: bounded, dataset-specific LongMemEval real runtime+baseline-score proof exists and bounded retrieval evidence now has a repo-owned local semantic judge lane; full-dataset maturity, full retrieval-distribution precision and upstream parity still stay open.
...
- companion continuity ownership/startup bundle:
...
- bounded graph-first startup/restore projection не имеет права подменять truth/control surface:
  - projection остаётся `truth_claim=false`;
  - при legacy `historical + hot` population или truncated SQL records preview
    (`sql_nodes_total > records_preview_count`) startup обязан уходить в
    `fallback_to_execctl_ledger`;
  - `validation.warnings.projection_preview_limited = true` само по себе не равно blocking-fallback:
    оно может означать только то, что compact preview по open/paused/closed/archive bucket был
    ужат, хотя полный SQL набор для control invariant уже дочитан и validated;
  - если validated path уже исключил deprecated/quarantined legacy debt из projection truth-lane,
    startup-facing `project_task_tree_summary` / `project_task_ledger_summary` должны честно
    surface-ить `excluded_legacy(...)`, чтобы оператор не искал этот объём только через raw
    validation JSON или ad hoc SQL;
  - live startup обязан surface-ить причину через `task_graph_projection_validation`, а не quietly
    продолжать как будто graph projection validated truth.
- threadless/shared lease не имеет права silently переходить к foreign thread:
  - foreign `continuity handoff` обязан fail-closed блокироваться;
  - foreign `continuity startup` не имеет права ребиндить такой lease;
  - background guard/read-only contour не имеет права quietly подменять `owner_thread_id`
    по stale restore hint.
- continuity ownership не имеет права зависеть от read-then-write окна:
  - `continuity handoff`, `continuity startup` и background guard обязаны сериализоваться одним
    advisory-lock contour по `(namespace_id, agent_scope)`;
  - stale restore не имеет права после ожидания этого lock contour перезаписывать уже сменившийся
    `source_event_id` или нового live owner;
  - current-session budget guard не имеет права брать raw `working_state_restore.thread_id` как
    trustable shortcut вне fresh lease-bound validation.
...
- same-thread same-scope competing handoff не должен молча steal-ить active line; если это реальный main-line redirect, нужен explicit promotion contract, иначе нужен distinct `AMAI_AGENT_SCOPE`.
...
- dashboard cards must expose `source_identity`, and `proof_observability.sh` must compare that identity with raw `/api/snapshot` for `snapshot_key`, `payload_root`, matrix, scope, source event, captured time, payload SHA and baseline/candidate ids;
- stale dashboard/exporter state вида `baseline pair ещё не materialized` после matrix proof path считается blocking proof failure.
...
Fresh refresh current work:
- Auto-open clean-surface launch scripts and proofs have been deleted from the repository (VS Code public bridge install/package/publish and all related `proof_vscode_*_bridge*` / `proof_vscode_code_chat_*` / `proof_continuity_frontdoor_*compact_chat*` proofs).
- Seamless transition is now defined as automatic continuity restore: every new/resumed chat surface must run `amai_continuity_startup` before any substantive reply. The required proof path is:
  - `./scripts/proof_onboarding.sh` validates that VS Code managed startup instruction (`.github/instructions/amai-continuity-startup.instructions.md`) and Hermes managed profile/SOUL.md are materialized and reference `amai_continuity_startup`;
  - `./scripts/proof_client_reconnect.sh` validates reconnect/fresh-chat fallback helpers for all supported clients;
  - `./scripts/proof_mcp_orphan_cleanup.sh` validates orphan MCP cleanup;
- No future claim about seamless transition may rely on `code --open-url`, `vscode://amai.amai-vscode-bridge/open-clean-chat`, or any other host-triggered clean-surface auto-launch without a project-level decision to reintroduce such a bridge.
...
Before-report workflow guard boundary:
- `proof_workflow_before_report.sh` is the required structural/execution-trace guard before any substantive report that claims the mandatory agent workflow was followed.
- `proof_specialist_signoff.sh` is the required signed tuple guard for `.amai/continuity/specialist-team-signoff.json`, `.amai/continuity/specialist-team-signoff-source.json` and `.amai/continuity/specialist-team-signoff-source.json.sig`.
- `materialize_specialist_signoff.sh` may write the signed tuple only after `amai verify workflow-trace` validates `agent-workflow-guard-v2`, stable workline identity, ordered stages, per-item team critique and team verification, final audit, proof bundle status and evidence binding.
- `proof_before_report.sh` must remain verify-only for the signoff tuple: it may refresh startup runtime state, but it must not silently re-materialize or rebind the signed signoff while proving readiness to report.
- `proof_before_report.sh` must also run the startup freshness/pin bundle: `proof_startup_redirect_freshness.sh` before and after the rest of the report gate, with startup-contract object SHA recomputation plus literal AGENTS pin match.
- `proof_before_report.sh` must also run the startup-runtime/reconnect hostile bundle: `proof_startup_runtime_state_fail_closed.sh`, `proof_continuity_reconcile_reconnect_hostile.sh`, and `proof_mcp_continuity_startup_stale_success_reconcile.sh`. A successful report gate must therefore prove not only static workflow shape, but also that stale runtime-state artifacts fail closed, local reconcile restores the startup state, reconnect assist is materialized only as diagnostic fallback, and a stale embedded MCP success is replaced by local startup payload plus same-session self-heal classification without operator action.
- `proof_before_report.sh` must also run the targeted Rust startup guards `cargo test --quiet startup_gate`, `cargo test --quiet continuity_startup_summary_fallback_gate`, and `cargo test --quiet inspect_startup_runtime_state_fails_closed_on_missing_execctl_active_lease_source_event_id`, so startup-gate semantics and missing lease-source-event drift cannot be skipped when the report bundle is used as the final readiness proof.
- `proof_before_report.sh` must also run the continuity ownership hostile bundle: targeted Rust guards for foreign-thread live-lease block, expired-lease restore block, no-live-thread-binding startup no-rebind, no-live-thread-binding guard no-rebind, threadless startup no-rebind, threadless guard no-rebind, lock-wait live-source/live-owner reload, validated current-session thread-hint reuse, plus `./scripts/proof_continuity_threadless_handoff_fail_closed.sh`. The report gate is not green if a foreign thread can still steal or silently rebind a shared/threadless continuity line through write, startup or guard paths, or if a stale restore can still survive through lock-wait or raw restore-thread reuse.
- `proof_before_report.sh` must also run `./scripts/proof_graph_first_startup_restore_projection.sh` inside the same verify-only bundle, because report readiness for this contour is not honest if graph-first startup projection regressions can still slip past the final gate.
- Startup runtime gate must keep `must_follow_startup_next_action=true` and `unrelated_work_allowed=false` even for `same_session_owner`; same-session ownership is not a bypass for the active-line return law.
- `verify workflow-trace` must reject self-attested proof strings: every `evidence_sha256` used by stage records, proof commands, specialists, role approvals and final bughunter pass must be a 64-hex non-placeholder hash backed by `evidence_manifest`, and that manifest must resolve each item to a regular non-symlink JSON file under `.amai/continuity/workflow-evidence/` whose bytes hash to the declared value, whose `artifact_version` is `workflow-evidence-v1`, whose `kind` matches the manifest item, and whose content matches the evidence-bearing record fields.
- Negative proof must keep forged-green paths red: placeholder hashes, syntactically valid but unbacked hashes, missing evidence files, manifest/file hash mismatch, directory evidence paths, symlink evidence paths, cross-kind evidence reuse, proof-command metadata mismatch and startup-gate unrelated-work escape are report blockers.
- This contour proves local workflow/report-gate integrity. It does not claim cryptographic non-repudiation against a compromised local signer or a compromised local workspace.

Fresh refresh 2026-06-02:
- `cargo run -- verify workflow-trace --fail-on-blocking-startup-gate` passed with `workflow_execution_trace_hash=4354a1b577984e6df10042a90b71fe73b625538bcf0a7b6a4a5ea3d4ab4bd828`, `evidence_manifest_hash=c4da92250ea0c6c4903eccdd6802a65e2379273a3d9cfbe650c06588bbe12316` and `consensus_fingerprint=85d1bbd47b32c045e5cc41eed6634b260e9a367657d49d851e90adccc3166ff6`.
- `./scripts/proof_workflow_before_report.sh` passed with hostile evidence-binding checks.
- `./scripts/proof_specialist_signoff.sh` passed after v2 signoff materialization.
- `proof_before_report.sh` now includes `proof_startup_redirect_freshness.sh` at both ends of the bundle, `cargo test --quiet startup_gate`, `cargo test --quiet continuity_startup_summary_fallback_gate`, `cargo test --quiet inspect_startup_runtime_state_fails_closed_on_missing_execctl_active_lease_source_event_id`, the full continuity ownership hostile bundle, `proof_startup_runtime_state_fail_closed.sh`, `proof_continuity_reconcile_reconnect_hostile.sh`, `proof_graph_first_startup_restore_projection.sh`, and `proof_mcp_continuity_startup_stale_success_reconcile.sh`, so startup-gate, startup pin freshness, missing lease-source-event drift, threadless/foreign ownership drift, lock-wait reload semantics, current-session thread-hint validation, graph-first startup projection, stale embedded MCP, runtime drift and reconnect hostile coverage are part of the report gate instead of optional side runs.
- `proof_mcp_continuity_startup_stale_success_reconcile.sh` now proves both the initial `project+repo_root` stale-success replacement and same-session follow-up calls (`repo_root` and `project`-only): after the first MCP success it injects a proof-only stale startup runtime artifact, requires `stale_embedded_mcp_session`, local startup payload replacement, `same_session_continuation_allowed=true`, `operator_action_required=false`, `mcp_reconnect_required=false`, and then requires later calls in the same embedded session to return without a second reconcile payload.
