...
- начат распил giant Rust modules:
  ...
  - ранний owner-test хвост `client-budget` контура теперь тоже co-located в `src/dashboard/dashboard_client_budget_diagnostics.rs` и `src/dashboard/dashboard_client_budget_support.rs`; `src/dashboard.rs` больше не держит рядом `exact_pair_status_override_*`, `exact_pair_status_metric_row_*`, `exact_pair_frozen_debt_metric_row_*`, `client_full_turn_savings_metric_row_*`, `current_live_turn_exact_pair_surfaces_zero_pair`, `client_live_limit_metric_row_*`, `client_live_context_metric_row_uses_last_request_window_pressure`, `client_turn_pressure_guard_requires_current_thread_binding` и ранние `client_budget_live_payload_surfaces_*` tests. Targeted owner-tests плюс `cargo check`, `agent_preflight`, `maintainability_gate` и `repo_hygiene_guard` подтверждают контрактную эквивалентность раннего same-meter/live-limit/live-payload surface после переноса, а корень `src/dashboard.rs` сокращён до `5058` строк.
