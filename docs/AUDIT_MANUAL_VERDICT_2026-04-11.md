...
- крупные bounded-context файлы остаются очень большими даже после уже выполненных split-pass:
  - `src/token_budget.rs` 26834
  - `src/postgres.rs` 33753
  - `src/observe.rs` 11037
  - `src/working_state.rs` 9781
  - `src/continuity.rs` 9472
  - `src/mcp.rs` 7559
...
- начат распил giant Rust modules:
  ...
  - postgres row-mapping helpers вынесены в `src/postgres/postgres_row_mapping.rs`; `src/postgres.rs` больше не держит рядом `*_record_from_row` functions и уже уменьшен до `33753` строк.
  - link/edge inserts (`MemoryLinkDecisionInsert`, `PendingLinkProposalInsert`, `MemoryEdgeInsert`, `MemoryConflictInsert`) вынесены в `src/postgres/postgres_link_inserts.rs`; `src/postgres.rs` больше не держит эти insert-структуры рядом с core-connect/boot логикой.
  - основной набор record/insert/internal-структур вынесен в `src/postgres/postgres_records.rs`, `src/postgres/postgres_inserts.rs`, `src/postgres/postgres_internal_structs.rs`.
  - continuity thread/index + startup types вынесены в `src/continuity/continuity_types.rs`.
  - continuity profile + compact-chat helper слой вынесен в `src/continuity/continuity_profile.rs` и `src/continuity/continuity_compact_chat_helpers.rs`.
  - startup runtime state helpers вынесены в `src/continuity/continuity_startup_runtime_state.rs`; `src/continuity.rs` уменьшен до `9472` строк.
  - observe client-budget cache helpers вынесены в `src/observe/observe_client_budget_cache.rs`; `src/observe.rs` уменьшен до `11422` строк.
  - working-state shell command helpers вынесены в `src/working_state/working_state_shell_commands.rs`; `src/working_state.rs` уменьшен до `10371` строк.
  - host current-thread control вынесен в `src/working_state/working_state_host_control.rs`.
  - workspace restore pack helpers вынесены в `src/working_state/working_state_restore_pack.rs`; `src/working_state.rs` уменьшен до `9781` строк.
  - restore pack persistence helpers вынесены в `src/working_state/working_state_restore_pack_persist.rs`; `src/working_state.rs` уменьшен до `9781` строк.
  ...
  - owner-tests working-state / live-file-hint контура теперь тоже co-located в `src/dashboard/dashboard_working_state_card.rs`; `src/dashboard.rs` больше не держит рядом `working_state_card_*`, `live_file_hints_*`, helper override/humanization tests и test-only import этого bounded context. Targeted owner-tests подтверждают контрактную эквивалентность waiting/pass logic, same-thread live-turn fallback, live file hint projection, preliminary handoff override и humanized reconciliation wording после переноса.
  ...
  - owner-tests service-card / external benchmark-Qdrant контура теперь тоже co-located в `src/dashboard/dashboard_service_cards.rs`; `src/dashboard.rs` больше не держит рядом benchmark-Qdrant/service-card test tail и test-only imports этого bounded context. Targeted owner-tests подтверждают контрактную эквивалентность last-success / no-cache / finished-error / live-running benchmark-Qdrant states и service-card filtering после переноса.
  ...
  - owner-tests live-response-latency compare контура теперь тоже co-located в `src/dashboard/dashboard_live_latency_compare.rs`; `src/dashboard.rs` больше не держит рядом `live_compare_card_*` test tail и test-only import этого bounded context. Targeted owner-tests подтверждают контрактную эквивалентность waiting/pass logic, live-readiness vs benchmark sample floor, stable fallback rows, rolling-window preference, previous-turn explanation и Amai-specific ignore-path для end-to-end response window после переноса.
  - dashboard shared metric/status evaluation contour вынесен в `src/dashboard/dashboard_metric_status_support.rs`; `src/dashboard.rs` больше не держит рядом observe-refresh stage summary, SLA reason/humanize helpers, status combiner/ranking logic, metric threshold comparators или cold-contour status helpers, которыми пользуются benchmark/service/runtime/payload owner-модули. Targeted tests подтверждают контрактную эквивалентность benchmark-Qdrant cards, service-card filtering, artifact-cleanup warning surface и shared `worst_status` semantics после split.
  ...
  - hero-owned historical frozen-debt contour тоже доведён до owner-complete в `src/dashboard/dashboard_hero_cards.rs`; `src/dashboard.rs` больше не держит рядом `reviewed_frozen_debt_export_*` и `historical_frozen_debt_*` helpers, которые materialize-ят lifetime review-only export и historical-only debt rows именно для hero-cards. Owner-tests теперь живут рядом с этими helper-ами и подтверждают контрактную эквивалентность historical tail и review-only export surfaces после split.
  - owner-tests раннего hero-контура теперь тоже co-located в `src/dashboard/dashboard_hero_cards.rs`; `src/dashboard.rs` больше не держит рядом `current_session_card_explains_raw_savings_vs_client_budget`, `hero_cards_explain_scope_and_strict_verified_fraction`, `hero_session_card_uses_waiting_status_before_verified_sample_exists`, `hero_cards_surface_client_limit_alignment_when_preview_is_present`, `hero_cards_alert_when_continuity_startup_burn_dominates_live_window`, `exact_model_component_delta_row_surfaces_top_same_meter_driver` и `rolling_window_card_surfaces_historical_startup_drag_when_current_session_is_profitable`. Targeted owner-tests подтверждают контрактную эквивалентность этих hero-инвариантов после переноса, а корень `src/dashboard.rs` сокращён до `70` строк.
  ...
  - ранний owner-test хвост `client-budget` и `current-session budget guard` контура теперь тоже co-located в `src/dashboard/dashboard_client_budget_diagnostics.rs`, `src/dashboard/dashboard_client_budget_support.rs` и `src/dashboard/dashboard_current_session_budget_guard.rs`; `src/dashboard.rs` больше не держит рядом `exact_pair_status_override_*`, `exact_pair_status_metric_row_*`, `exact_pair_frozen_debt_metric_row_*`, `client_full_turn_savings_metric_row_*`, `current_live_turn_exact_pair_surfaces_zero_pair`, `client_live_limit_metric_row_*`, `client_live_context_metric_row_uses_last_request_window_pressure`, `client_turn_pressure_guard_requires_current_thread_binding`, `client_budget_root_cause_payload_*`, `client_budget_live_payload_surfaces_hourly_burn_reply_prefix`, `client_budget_live_payload_prefers_live_personal_agent_reply_prefix`, `current_session_budget_guard_*` и helper tests для same-thread label/host-control selection. Targeted owner-tests плюс `cargo check`, `agent_preflight`, `maintainability_gate` и `repo_hygiene_guard` подтверждают контрактную эквивалентность раннего same-meter/live-limit/live-payload + current-session guard surface после переноса, а корень `src/dashboard.rs` сокращён до `70` строк.
  ...
  - owner-tests install/browser context, payload alias, monitoring URL, top-card overview и artifact-cleanup runtime контура теперь тоже co-located в `src/dashboard/dashboard_context.rs`, `src/dashboard/dashboard_payload.rs`, `src/dashboard/dashboard_card_support.rs`, `src/dashboard/dashboard_overview.rs` и `src/dashboard/dashboard_runtime_support.rs`; `src/dashboard.rs` больше не держит рядом `browser_url_rewrites_unspecified_v4`, `dashboard_payload_exposes_live_compare_card_alias_from_top_cards`, `monitoring_url_reuses_dashboard_host`, `top_cards_split_live_retrieval_from_real_workline`, `artifact_cleanup_warning_*` / `artifact_cleanup_card_*` tests и test-only helper `synthetic_machine_summary`. Targeted owner-tests подтверждают контрактную эквивалентность этих bounded contexts после переноса, а корень `src/dashboard.rs` сокращён до `70` строк без нового contract drift.
  - owner-tests client-turn pressure и refresh/format contracts теперь тоже co-located: `client_turn_pressure_*` переехали в `src/dashboard/dashboard_client_budget_support.rs`, `format_ms_*`/`elapsed_label_*`/`compare_time_pair_*` — в `src/dashboard_format.rs`, а `dashboard_html_*` refresh/rename contract — в `src/dashboard/dashboard_renderer.rs`; `src/dashboard.rs` больше не держит рядом эти owner-tests. Targeted tests подтверждают контрактную эквивалентность pressure guard, format surface и HTML refresh/rename contract, а корень `src/dashboard.rs` зафиксирован на `70` строках по `wc -l`.
  ...
  - token-budget context-pack thread binding helpers вынесены в `src/token_budget/context_pack_thread_bindings.rs`; `src/token_budget.rs` уменьшен до `27686` строк.
  - token-budget rate-limit/parse surfaces вынесены в `src/token_budget/client_rate_limit_surfaces.rs`; `src/token_budget.rs` уменьшен до `27686` строк.
  - token-budget config/model/defaults вынесены в `src/token_budget/token_budget_models.rs`; `src/token_budget.rs` больше не держит core config structs, profile structs, default version functions и dashboard cache structs рядом с runtime-логикой; `src/token_budget.rs` уменьшен до `27686` строк.
  - token-budget exact client-limit runtime / live-meter contour вынесен в `src/token_budget/client_limit_runtime.rs`; `src/token_budget.rs` больше не держит рядом codex app-server rate-limit query, exact sample persistence, hourly burn/trend analysis и live-meter JSON builders; `src/token_budget.rs` уменьшен до `26834` строк.
  - observe cache meta/stale helpers вынесены в `src/observe/observe_cache_meta.rs`; `src/observe.rs` уменьшен до `11422` строк.
  - observe local http helpers вынесены в `src/observe/observe_local_http.rs`; `src/observe.rs` уменьшен до `11422` строк.
  - observe client-budget runtime/compact helpers вынесены в `src/observe/observe_client_budget_runtime.rs`; `src/observe.rs` уменьшен до `11422` строк.
  - observe thread-bound snapshot / merge / auto-thread-hint contour вынесен в `src/observe/observe_thread_bound.rs`; `src/observe.rs` больше не держит рядом thread-bound snapshot assembly, compact request cache selection, merge/meta helpers и strict auto-thread binding rules; `src/observe.rs` уменьшен до `11037` строк.
