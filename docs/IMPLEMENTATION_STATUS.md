...
- refresh 2026-05-29: project/root binding contract уточнён после фикса nested-folder bug:
  path-based startup теперь должен резолвить child/descendant `repo_root` hint в уже bound canonical
...
- targeted runtime coverage for child/relocated repo-root hints and automatic `continuity`
  namespace creation in `src/postgres/postgres_runtime_tests.rs` / `src/continuity.rs`;
...
- refresh 2026-05-29: external benchmark unit drift закрыт после восстановления benchmark configs:
  `ann-benchmarks` launch-marker assertion теперь берёт текущий `ann_qdrant_launch_marker()`,
  ANN HDF5 parser стабилизирует derived float metrics и сохраняет latency/QPS parsing даже без
  ground-truth dataset, возвращая `recall = None`; `cargo test --locked external_benchmark`
  прошёл `116 passed`.
...
- The official judge lane now also fail-closes on non-official API endpoints by default: `https://api.openai.com/v1` is the required upstream base URL for official provenance, while local fake-provider proofs must opt in explicitly and now materialize non-official provenance instead of silently looking upstream-eligible.
