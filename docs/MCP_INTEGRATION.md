...
| `kimi-code` | managed block в `AGENTS.md` + `.kimi-code/mcp.json` + `~/.kimi-code/config.toml` `[[hooks]]` SessionStart hook | 2 | managed artifact, SessionStart hook и прямой Rust startup проверены (`proof_client_auto_startup_kimi_code.sh`; hook реально исполняет canonical startup с точным Kimi session id); live client/session proof отсутствует |
...
4. Reconnect helper (`./scripts/reconnect_local.sh --client <client>`) остаётся только диагностическим fallback на случай повторного transport/runtime failure.

Для Tier-2 клиентов model вызывает startup по system-prompt seed, поэтому даже если клиент не умеет `initialize.instructions`, первая user-visible реплика не должна начаться без свежего continuity payload.
...
- если project startup contract artifact отсутствует, не читается или не проходит hash-check,
...
- onboarding теперь materialize-ит и отдельный project JSON artifact:
  `.amai/onboarding/project-chat-startup-contract.json`;
...
- если нужно inspect-ить этот runtime artifact в конкретном project root, а не в самом
...
- `proof_remote_onboarding.sh` проверяет тот же набор через `onboard_remote_client.sh`, включая local startup artifacts и `OpenClaw` project-bound agent/project path.
