...
- operational baseline проекта;
- proof/gate discipline;
- fail-closed и reconcile laws;
- как новый contour должен materialize-иться честно.

Он не заменяет общий target-state roadmap памяти.

Если агент пытается понять:
- что уже реализовано;
- что ещё нет;
- какой этап следующий;
- чем current-state отличается от target-state;

сначала нужно открыть [AGENT_START_HERE.md](AGENT_START_HERE.md), а затем [AMAI_GLOBAL_MEMORY_ROADMAP.md](AMAI_GLOBAL_MEMORY_ROADMAP.md).
...
- short name: `Amai`
- текущий path задаётся как active `<repo-root>` этого standalone проекта

## Incident bundle discipline for cross-store indexing failures

Contour `index_project.qdrant_postgres_failure_verdict` больше не может ограничиваться только
`required_action=manual_cross_store_investigation_required` в логе или error text.

Для всех branch-ей, где cross-store recovery требует ручного расследования
(`cross_store_consistency_unknown_commit_outcome` и
`cross_store_inconsistent_after_compensation_failure`), runtime теперь обязан materialize-ить
durable incident bundle на disk:
- default path:
  `state/incidents/qdrant-postgres-remediation/*.json`
- test/debug override:
  env `AMAI_QDRANT_POSTGRES_REMEDIATION_DIR`
- live observability stage:
  `index_project.qdrant_postgres_manual_recovery_bundle`

Этот bundle обязан нести как минимум:
- `relative_path`
- `document_id`
- `failure_mode`
- `failure_phase`
...
- `consistency_state`
- `required_action`
- operator checklist для reconcile decision

Truthful reading:
- verdict log отвечает за detection/classification;
- remediation bundle отвечает за surfaced operator handoff;
- первый control/read surface теперь должен идти через read-only HTTP inspect path
  `/api/remediation-bundles` для inbox list и `/api/remediation-bundles/item?file_name=...` для
  single-artifact detail; оба path читают только bundle artifacts и не имеют права выполнять
  retry/reconcile/cleanup;
- без bundle path manual-investigation branch считается operationally недоведённым даже если tests
  зелёные.
...
- launcher `scripts/run_mcp_stdio.sh` и `scripts/run_mcp_stdio.ps1` должны идти через
  `cargo run --release --quiet -- mcp serve`, если `cargo` доступен;
- fallback на `target/release/amai` допустим только как degrade path на машинах без `cargo`;
- shell launcher не должен писать debug/log строки в stdout до JSON-RPC handshake; любой диагностический
  вывод до старта сервера уходит только в log/stderr, иначе stdio MCP stream считается повреждённым;
- product proof для этого слоя:
  `./scripts/proof_mcp_launcher_freshness.sh`.

Proof-refresh 2026-04-25: `./scripts/proof_mcp_launcher_freshness.sh`
прошёл зелёно и подтвердил, что shared stdio launcher выбирает
`cargo run --release --quiet -- mcp serve`, а не stale release binary, когда
`cargo` доступен. Это proof launcher-а, а не live-client e2e.
...
- для локального Linux path он теперь обязан materialize-ить managed `systemd --user` unit `amai-stack.service`, чтобы stack поднимался при старте user manager без ручного `./scripts/bootstrap_stack.sh`;
- это не headless reboot guarantee: для автоподъёма без входа пользователя нужен отдельный `linger`/system-service lane и отдельный proof.
- proof-refresh 2026-04-25 подтвердил bounded user-manager contract через `proof_stack_autostart.sh`, `proof_bootstrap_volume_dirs.sh` и `proof_onboarding.sh`; unattended boot без login остаётся отдельным gap.
...
Proof-refresh 2026-04-25: `./scripts/proof_memory_bridge_search.sh` и
`cargo test --quiet --bin memory` прошли зелёно. Это подтверждает release bridge
output/explainability contract, но не заменяет отдельные retrieval-quality proofs.
...
- тот же `remote_ssh` contour поддерживается и для `Cursor`, `Codex`, `Claude Code`, `Hermes` и `OpenClaw`.
...
- `AMI_STACK_BIND_HOST`
  - bind-адрес опубликованных Docker port mappings;
  - канонический hardened default для local stack: `127.0.0.1`;
- `AMI_PROMETHEUS_PORT` и `AMI_GRAFANA_PORT`
  - локальные порты monitoring profile.
- `AMI_PROMETHEUS_IMAGE` и `AMI_GRAFANA_IMAGE`
  - должны быть pinned tags, а не `latest`, чтобы monitoring profile был воспроизводимым.
- `AMI_NATS_AUTH_MODE`
  - `disabled` для local/dev path;
  - `password` materialize-ит named-user auth в rendered `tmp/nats/server.conf`;
- `AMI_NATS_USER` и `AMI_NATS_PASSWORD`
  - обязательны только когда `AMI_NATS_AUTH_MODE=password`;
  - при этом `AMI_NATS_URL` тоже должен нести credentials, если клиент реально подключается к auth-required NATS.
- `AMI_SECURITY_PROFILE`
  - `default` сохраняет local/dev path без обязательного TLS/auth;
  - `hardened` включает обязательные требования к TLS/auth и проверяется через `./scripts/proof_security_hardening_contract.sh`.
- `AMAI_EXEC_SUPPRESS_BUILD_NOISE`
  - `1` (default) подавляет cargo build chatter для operator commands, если используется `./scripts/amai_exec.sh`;
  - `0` включает полный build output.
- `AMI_POSTGRES_CERTS_DIR`
  - каталог с `server.crt` и `server.key` для TLS server-side;
  - используется в compose-монте `/etc/postgresql/certs`.
- `AMI_MINIO_CERTS_DIR`
  - каталог с `public.crt` и `private.key` для MinIO;
  - используется в compose-монте `/root/.minio/certs`.
- `AMI_MINIO_SCHEME`
  - `http` для local/dev path;
  - `https` обязателен для `AMI_SECURITY_PROFILE=hardened`.
...
Hardened deployment contract для local stack:
- положить TLS материалы в `AMI_POSTGRES_CERTS_DIR` и `AMI_MINIO_CERTS_DIR`;
- выставить:
  - `AMI_SECURITY_PROFILE=hardened`
  - `AMI_MINIO_SCHEME=https`
  - `AMI_S3_ENDPOINT=https://...`
  - `AMI_POSTGRES_DSN` и `AMI_APP_POSTGRES_DSN` с `sslmode` ≠ `disable`;
- перед запуском compose прогнать:
  - `./scripts/render_postgres_config.sh`
  - `./scripts/proof_security_hardening_contract.sh`
- затем `./scripts/bootstrap_stack.sh`.
...
- `AMI_APP_POSTGRES_DSN`
  - read-only runtime path для status/observe/read-side surfaces;
- proof `./scripts/proof_app_db_role_read_only.sh`
  - обязан подтверждать, что `SELECT` через app-role проходит, а `INSERT` в `ami` schema режется.
...
- `working_state`
  - фиксирует текущее рабочее состояние между рабочими поверхностями:
    - активную цель;
    - последние retrieval-запросы;
    - активные файлы;
    - текущую непрерывную рабочую сессию.
- `chat_start_restore`
  - компактный startup pack для первого содержательного ответа новой рабочей поверхности;
...
- `working_state_restore` теперь execution-aware:
  ...
  - `execctl_resume_contract` и `execctl_resume_contract_summary` теперь machine-readable поднимают
    `required_return_task`, чтобы новая рабочая поверхность или MCP-клиент видел obligation к возврату явно;
...
- direct CLI path через `cargo run --manifest-path <amai-repo-root>/Cargo.toml -- ...` тоже должен поднимать `Amai .env` без ручного `cd` в repo root;
...
- использовать alias-path вроде `../Art` вместо уже зарегистрированного абсолютного `repo_root` запрещено: это не новый проект, а конфликт регистрации.
- использовать вложенную подпапку уже bound project root как новый проект запрещено: path-based startup
  должен трактовать её как child/descendant hint и вернуть canonical project root;
- если переданный root является parent hint для ровно одного child/relocated binding, он резолвится в
  этот project и возвращает canonical `ami.projects.repo_root`; если таких child bindings несколько,
  resolve обязан fail-closed как ambiguous;
...
- если один user request несёт несколько явных подпунктов, startup/restore теперь обязан surface-ить
  и отдельный `required_task_set`, чтобы runtime не схлопывал multi-task obligation до одного
  headline после первого частичного успеха.
- тот же startup execution gate должен нести `required_task_set_count`,
  `required_task_set_present` и `must_preserve_required_task_set = true`, когда набор не пуст.
...
- тот же artifact теперь несёт `startup_contract_sha256`, а managed instructions поднимают тот же
  ...
  startup contract.
- важно: `startup_contract_sha256` не равен `sha256sum` всего файла
  `.amai/onboarding/project-chat-startup-contract.json`;
  scope этого pin-а = `startup_contract object only`, то есть хэш считается только по вложенному
  объекту `startup_contract`, а не по внешней envelope-обёртке artifact-файла.
- значит изменения верхнеуровневых полей artifact envelope сами по себе не должны считаться
  drift этого pin-а, пока вложенный `startup_contract` не изменился.
...
- `rotate_status_labels` обязаны использовать канонический normalized same-thread advisory set
  из `dashboard_client_budget_support`;
  текущий набор: `["сожми текущий чат", "сожми текущий чат сейчас"]`;
  этот список нужно читать как non-binding human-readable snapshot канонического shared
  advisory source, а не как отдельный policy/truth layer;
  если shared set меняется, этот документ нужно обновлять как snapshot, а не трактовать
  локальный список как независимый source of truth;
...
- после такой настройки новая рабочая поверхность может честно рассчитывать на auto-start только если
...
- если уже после restore пользователь явно переключил задачу, active file или выделенный
  проверяемый фрагмент, это новый active workline и его нельзя затирать старым headline из
  startup/handoff;
- в таком live user-redirect contour агент обязан сначала materialize-ить новый
  `continuity_handoff`, чтобы старая линия ушла в `pending_return_queue`, и только потом
  отвечать по новому запросу;
- `startup_next_action` и `required_return_task` не дают права игнорировать явный новый
  пользовательский redirect внутри того же чата; они описывают незавершённые обязательства,
  а не override для нового active user request.
...
После proof-refresh 2026-04-25 этот же proof также подтверждает user-manager
autostart install output: `Amai stack autostart ready: amai-stack.service`.
Это не является proof-ом headless reboot без login/linger.

`./scripts/proof_remote_onboarding.sh` теперь идёт как multi-client matrix:
он прогоняет `onboard_remote_client.sh` для `VS Code`, `Cursor`, `Codex`,
`Claude Code`, `Hermes` и `OpenClaw`, проверяет `launcher_mode: remote_ssh`,
client-native startup artifacts и симметричный cleanup после `disconnect_local.sh`.
...
`VS Code`, `Cursor`, `Codex`, `Claude Code`, `Hermes` и `OpenClaw`: proof
создаёт orphaned `amai mcp serve`, запускает `reconnect_local.sh`, убеждается,
...
- для stage-close proof урезанный benchmark режим запрещён: минимум `warmup >= 3`, `iterations >= 20`;
- если benchmark contour публикует результат на dashboard, после прогона обязателен и dashboard-check этого результата.
...
Для self-contained repo-local contour без внешнего corpus:
...
Этот contour:
- использует только `config/cold_benchmark_self_contained.toml` и один repo `Amai`;
- нужен для reproducibility / clean-machine story;
- намеренно помечается как `proof`, поэтому не имеет права перетирать `latest_cold_path_benchmark`.

Для optional NATS auth hardening:
...
Что это materialize-ит:
- rendered runtime config теперь живёт в `tmp/nats/server.conf`, а не в статичном committed файле;
- `AMI_NATS_AUTH_MODE=disabled` сохраняет совместимый local/dev path;
- `AMI_NATS_AUTH_MODE=password` включает named-user auth через `AMI_NATS_USER` и `AMI_NATS_PASSWORD`;
- proof `./scripts/proof_nats_auth_render.sh` обязан подтверждать оба режима.

Для end-to-end security contract:

```bash
./scripts/proof_security_hardening_contract.sh
```

Этот proof требует, чтобы при `AMI_SECURITY_PROFILE=hardened`:
- `AMI_POSTGRES_DSN` и `AMI_APP_POSTGRES_DSN` использовали `sslmode` не `disable`;
- `AMI_S3_ENDPOINT` был `https://...`;
- `AMI_S3_ACCESS_KEY/AMI_S3_SECRET_KEY` были отличны от default/change_me;
- `AMI_NATS_AUTH_MODE=password` и `AMI_NATS_URL` содержал credentials.
...
- `Поведение при сбоях` больше не живёт в live service-card surface: это proof/policy contour, а не live runtime карточка оператора;
- Prometheus:
...
- `Правильное продолжение` больше не живёт в live service-card surface: это proof contour последней explicit continuity verification, а не live runtime карточка;
...
Cold/full path нужно оценивать отдельно через оба контура:
- `./scripts/proof_performance.sh` для micro cold retrieval contour;
- `./scripts/proof_cold_benchmark.sh` для proof full-cold contour;
- `./scripts/proof_cold_benchmark_self_contained.sh` для repo-local self-contained contour;
- `./scripts/proof_cold_benchmark_canonical.sh` для canonical large repo-pool contour, который имеет право обновлять `Cold End-to-End Benchmark / latest_cold_path_benchmark`.
Проверять только один из них недостаточно.
...
External long-term memory имеет отдельные proof lanes:
...
Первый proof закрывает dataset/source prep, adapter workspace, normalized cases и synthetic runtime/score smoke. Второй proof закрывает только bounded, dataset-specific LongMemEval runtime+baseline-score evidence: real requests, predictions, status, metrics and score for `AMAI_EXTERNAL_MEMORY_REAL_LIMIT` cases. Relaxed retrieval retry в этом proof означает только retrieval participation, а не semantic precision. Ни один из этих proof-ов не доказывает official upstream scorer parity или full external benchmark-grade maturity.
...
Fresh status note 2026-04-24:
- these verdict counts describe the intended canonical eval semantics and are again backed by a fresh green local proof bundle;
- `./scripts/proof_benchmark_contamination_preflight.sh` is green and covers the strict-heavy fail-closed path for latency-sensitive benchmarks;
- `./scripts/proof_mcp_task_matrix.sh` is green after strict-heavy contamination preflight and explicit failed-run artifact handling;
- `./scripts/proof_observability.sh` is green; dashboard `MCP task matrix compare` no longer shows `ещё нет данных` after the MCP matrix proof path.
...
- `adjustment_registry`
  ...
  - по умолчанию registry ищется в repo-local
    `<repo-root>/state/token_adjustment_registry.json`;
...
- `reconciliation_previews` теперь тоже каноничны:
  ...
  - repo-local sources теперь допускаются как operator-safe truth bindings:
    - `<repo-root>/state/provider_usage_export.json`
    - `<repo-root>/state/provider_invoice_export.json`
    - `<repo-root>/state/provider_rate_card.json`
    - `<repo-root>/state/infra_cost_profile.json`
...
- если live client-turn pressure уже высокий, остаток основного лимита низкий, а доля
  ...
  доступен и не исчерпан verified failure;
- этот warning-set нужно читать как non-binding operator-facing projection канонического shared
  advisory source, а не как второй независимый список policy-labels внутри `OPERATIONS.md`;
- тот же operator-facing warning теперь обязан включаться и раньше, если exact full-turn pair
  ...
  честно доказать нельзя без same-thread compaction и короткого exact pair;
- и даже если exact full-turn pair уже materialized, warning обязан включаться раньше, когда
  сам full-turn уже стал burn-dominant, а доля `Amai в полном live-turn` остаётся
  микроскопической; иначе dashboard честно знает точный tiny-share, но operationally опаздывает
  с same-thread compaction guard;
- такой warning обязан не просто красить статус, а давать прямое follow-up действие:
  сначала открыть same-thread control текущего giant thread, а handoff/new clean work surface
  держать только как fallback после подтверждённого провала same-thread control.
...
- Тот же managed law теперь обязан быть time-fresh:
  если age последнего machine-readable guard превышает `10` секунд, клиент не имеет права
  полагаться на старый meter и должен сначала обновить guard, а уже потом отвечать.
- для runtime и automation теперь нужен отдельный machine-readable inspect-layer:
  `cargo run -- observe client-budget-guard`;
- этот contour обязан поднимать прямые булевы и action-oriented поля
  (`should_rotate_chat_now`, `should_rotate_chat_soon`, `full_turn_savings_proven`,
  `next_action`, `last_request`, `client_limits`), чтобы rotate-chat automation не
  зависела от визуального чтения dashboard card.
- human-facing rows `Последний запрос клиента` и `Лимит клиента сейчас` теперь тоже обязаны
  нести краткую visible freshness-метку `· raw HH:MM:SS MSK`; этого требует operator trust,
  чтобы человек видел прямо на карточке, что цифра взята из конкретного raw `token_count`,
  а не из неясного кеша или пересказанного internal state.
...
- версия правил и retention profile приходят из [config/observability.toml](../config/observability.toml) и materialize-ятся в `_observability`.
...
- список путей, TTL и `keep_latest` живут в [config/observability.toml](../config/observability.toml);
...
`critical=0`, `memory_matrix_tasks_failed=0`, тремя MCP prompts
`amai-context-pack / amai-continuity-startup / amai-onboarding` и token savings
`83.69098712446352%`. Это proof MCP runtime contract, не live-client UX.
...
Этот proof теперь проверяет remote `ssh` launcher matrix для `VS Code`, `Cursor`,
`Claude Code`, `Codex`, `Hermes` и `OpenClaw`.
...
- SLA-оценку по [observability.toml](../config/observability.toml).
...
- `/api/dashboard-live-summary`
  - lightweight coherent live bundle для `headline`, hero-card `Экономия токенов за текущую сессию`
    и верхних top-cards;
- `/api/snapshot`
...
- не поднимает proof-only карточку `Правильное продолжение` в live operator surface, чтобы не смешивать статичный explicit proof с живыми runtime сигналами.

Если оператор видит высокий `%CPU` у `code --type=zygote`, это не считается достаточным доказательством реального текущего burn.
Для VS Code / Electron authoritative truth теперь нужно снимать так:

```bash
./scripts/vscode_live_cpu_truth.sh
```

Контракт этого контура:
- `ps %CPU` у long-lived `zygote` и Electron utility процессов считается lifetime-biased advisory, а не authoritative истиной;
- `code --status` считается authoritative для активного окна, `extension-host` и MCP child contour;
- windowed live sample по `/proc/<pid>/stat` считается authoritative для реального CPU burn;
- для `observe serve` authoritative pair = тот же windowed live sample + `/healthz`.
...
- [config/prometheus/rules/alerts.yml](../config/prometheus/rules/alerts.yml)
- [config/grafana/dashboards/amai_stack.json](../config/grafana/dashboards/amai_stack.json)
- [scripts/render_monitoring_config.sh](../scripts/render_monitoring_config.sh)
...
- machine summary contour на human dashboard больше не имеет права дёргать privileged hardware
  ...
  - `headline`, hero-card `Экономия токенов за текущую сессию` и top-cards больше не имеют права
    жить на независимых poll-контрах:
    - steady-state UI обязан обновлять их одним coherent live bundle, а не тремя разными
      запросами с независимым drift;
    - bootstrap snapshot разрешён только как стартовая заливка страницы;
    - после startup summary/top-cards/hero-card не имеют права ждать только `focus` или
      `visibilitychange`, иначе агент унаследует ложный stale UX даже при свежем backend;
