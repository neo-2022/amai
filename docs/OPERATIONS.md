...
- если live client-turn pressure уже высокий, остаток 5h лимита низкий, а доля
  ...
  operator-facing warning `сожми текущий чат` или `сожми текущий чат сейчас`, если
  host current-thread control доступен и не исчерпан verified failure;
...
- такой warning обязан не просто красить статус, а давать прямое follow-up действие:
  сначала открыть same-thread control текущего giant thread, а handoff/new chat держать только
  как fallback после подтверждённого провала same-thread control.
