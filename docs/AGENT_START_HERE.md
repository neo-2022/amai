...
8. После каждого значимого подшага:
  ...
  - if embedded MCP startup succeeds but the runtime state has a stale startup-contract SHA or
    ...
    `stale_embedded_mcp_session`, run local `./scripts/continuity_startup.sh`, continue from the
    self-healed local payload in the same session, and keep reconnect only as diagnostic fallback
    for repeated transport/runtime failure;
