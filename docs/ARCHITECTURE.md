...
- exact root, ancestor root и relocated/descendant hints должны возвращать canonical project record
  из `ami.projects`, а не alias-row из `ami.project_repo_roots`;
- подпапка внутри уже bound root является repo-root hint для существующего project, а не поводом
  заводить второй project с похожим именем;
- parent/root hint, под которым найдено несколько child project bindings, считается ambiguous и
  обязан fail-closed остановиться;
- continuity startup должен сначала доверять canonical `repo_root` hint, а stale client-side
  `project` code принимать только если он не конфликтует с resolved project;
...
- `project register`
  - регистрирует project root, `default` namespace и `continuity` namespace;
  ...
  - если старый child-root позже становится alias после регистрации parent-root для того же project,
    path-based startup должен резолвить child hint обратно в canonical parent project.
- `namespace ensure`
