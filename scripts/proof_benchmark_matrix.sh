...
printf '%s\n' "$list_output" | rg '^- LiveMCPBench \(live_mcpbench\) — следующий обязательный приоритет$' >/dev/null
...
printf '%s\n' "$coverage_output" | rg '^- Следующий обязательный приоритет: 2$' >/dev/null
printf '%s\n' "$coverage_output" | rg '^Следующие приоритеты:$' >/dev/null
printf '%s\n' "$coverage_output" | rg 'LiveMCPBench \(live_mcpbench\)' >/dev/null
printf '%s\n' "$coverage_output" | rg 'MCP-Universe \(mcp_universe\)' >/dev/null
...
printf '%s\n' "$live_mcp_output" | rg 'multi-server tool tasks' >/dev/null
