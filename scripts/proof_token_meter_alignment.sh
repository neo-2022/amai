...
assert contract["client_limit_meter_alignment_version"] == "client-limit-meter-alignment-v1", contract
assert agent_cycle["contract"]["client_limit_meter_alignment"]["model_version"] == "client-limit-meter-alignment-v1", agent_cycle
...
for scope in ("current_session", "rolling_window", "lifetime"):
    ...
    assert alignment["model_version"] == "client-limit-meter-alignment-v1", preview
...
for scope in ("current_session", "rolling_window", "lifetime"):
    ...
    assert alignment["model_version"] == "client-limit-meter-alignment-v1", scope_payload
