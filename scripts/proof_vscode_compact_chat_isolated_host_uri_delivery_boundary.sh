...
if [[ -z "${chatgpt_bundle}" || ! -d "${chatgpt_bundle}" ]]; then
  echo "proof_vscode_compact_chat_isolated_host_uri_delivery_boundary: openai.chatgpt bundle not found" >&2
  exit 1
fi
...
code --user-data-dir "${user_dir}" --extensions-dir "${ext_dir}" --new-window "${repo_root}" >/dev/null 2>&1 || true
sleep 8

prompt_file="${tmp_root}/prompt.txt"
result_file="${tmp_root}/result.json"
printf 'isolated uri boundary probe %s\n' "$(date -Is)" > "${prompt_file}"
...
if [[ -f "${result_file}" ]]; then
  echo "proof_vscode_compact_chat_isolated_host_uri_delivery_boundary: isolated host unexpectedly wrote bridge result file" >&2
  cat "${result_file}" >&2
  exit 1
fi
...
if [[ -z "${exthost_log}" || ! -f "${exthost_log}" ]]; then
  echo "proof_vscode_compact_chat_isolated_host_uri_delivery_boundary: missing exthost log" >&2
  exit 1
fi
if [[ -z "${codex_log}" || ! -f "${codex_log}" ]]; then
  echo "proof_vscode_compact_chat_isolated_host_uri_delivery_boundary: missing Codex log" >&2
  exit 1
fi

rg -Fq "ExtensionService#_doActivateExtension openai.chatgpt" "${exthost_log}"

printf 'proof_vscode_compact_chat_isolated_host_uri_delivery_boundary: ok (%s)\n' "${tmp_root}"
