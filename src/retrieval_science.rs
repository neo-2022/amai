...
mod tests {
    ...
    #[test]
    fn suite_metadata_loads_memory_retrieval_semantic_evidence_suite() {
        let suite = suite_metadata("memory_retrieval_semantic_evidence").expect("suite metadata");
        assert_eq!(
            suite["suite_version"].as_str(),
            Some("external-memory-local-retrieval-judge-v2")
        );
        assert_eq!(
            suite["manifest_path"].as_str(),
            Some("docs/MEMORY_BENCH_RUNBOOK.md")
