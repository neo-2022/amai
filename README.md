modified_at: 2026-03-20 22:06 MSK
Ручная сверка guide/docs: 2026-03-20 22:06 MSK
...
## Шаг 1. Проверить, подходит ли машина
...
- если в конце написано `Итог: машина подходит`
  - можно спокойно переходить к установке;
- если написано `Итог: машина подходит с оговорками`
  - установка возможна, но без тяжёлых ожиданий;
- если написано `Итог: машина не подходит для этого режима`
  - лучше не продолжать этот профиль и либо взять более сильную машину, либо попробовать `lite_vps`.
...
## Шаг 3. Что делать во время установки

Когда `Amai` закончит проверку и спросит подтверждение, просто напишите:

```text
ДА
```

После этого установка продолжится сама.
