...
- живые retrieval-карты по типам:
  - `mix`
  - `hot`
  - `cold`;
- в каждой такой карте крупным числом показывается текущий live-показатель прямо сейчас;
- в каждой такой карте отдельными строками сразу видны:
  - `Эталон`;
  - `Сейчас`;
  - `P99`;
  - `Max`;
  - `sample_count`;
- англицизмы и спецтермины теперь расшифровываются подсказкой при наведении на `?`;
