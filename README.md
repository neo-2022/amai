...
`Amai` — это внешний memory/continuity инструмент для ИИ-агентов.
Его задача — не давать агенту начинать каждую сессию с нуля, путать проекты и заново собирать
контекст по кускам из чатов, файлов и случайных заметок.

Коротко:
- `Amai` хранит рабочую линию, память, restore и project isolation отдельно от чата;
- IDE, окно, CLI или MCP-клиент для него — это только поверхности доставки;
- source of truth живёт в `Amai`, а не в одной конкретной сессии.

## Текущий статус

Проект **в разработке**.

При этом уже сейчас можно использовать `Amai` в `VS Code`:
- public install contour есть;
- publish/install contour для `VS Code` отработан и проверен;
- extension публикуется и ставится через `OpenVSX`.

Важно:
- на текущем этапе самым зрелым и проверенным клиентом является именно `VS Code`;
- остальные приложения и клиентские контуры будут добавляться по мере развития проекта.
...
- [AMAI_SYSTEM_OVERVIEW.md](docs/AMAI_SYSTEM_OVERVIEW.md)
...
А [AMAI_SYSTEM_OVERVIEW.md](docs/AMAI_SYSTEM_OVERVIEW.md) нужен затем, чтобы не собирать
общий замысел `Amai` по кускам из разных файлов. В нём одной картиной сведены:
- память и continuity;
- задачи и возвраты;
- интеграции, платформы, клиенты, хосты и ОС;
- научные advisory-идеи вроде `Markov`, `Poisson`, `regression explain` и `KAN`;
- главный закон, что verified truth важнее красивых расчётов.

А канонический порядок реализации нужно брать не из обзора, а только из
...
Важно:
- transcript/history чата для `Amai` — это input/projection surface;
- сама память, continuity и восстановление рабочего состояния живут в `Amai` как в source of
  truth.
...
## Как `Amai` продолжает работу в новой рабочей поверхности
...
- новая рабочая поверхность поднимает не только “что решили в конце”, но и:
  - текущую цель;
...
Это короткий готовый блок, который уже можно считать восстановленным рабочим контекстом для
первого содержательного ответа новой рабочей поверхности.
В нём уже лежат:
...
То есть новая рабочая поверхность теперь должна видеть не только summary “о чём проект”, а уже
компактный готовый стартовый контекст для продолжения работы.
...
- новый клиент или новая рабочая поверхность должны видеть это как обязательство к возврату, а не
  как свободный текстовый совет.
...
Важно: continuity должна переживать не только новую рабочую поверхность, но и смену окна, IDE и
локализации проекта.
...
- автоматический startup/restore в каждой новой рабочей поверхности.
...
- руками вызывать restore в каждой новой рабочей поверхности не должно быть нужно;
...
- `continuity startup`
  - печатает human-readable стартовую сводку для новой рабочей поверхности;
  ...
  - этот path должен вызываться клиентом автоматически при старте новой рабочей поверхности для уже подключённого проекта;
  - пользователь не должен руками запускать его в каждом новом окне;
  - теперь в этом выводе печатается и готовый `prompt_text`, чтобы новая рабочая поверхность получала не только summary, но и прямой compact restore для первого ответа;
...
- `continuity restore`
  ...
  - внутри `working_state_restore` теперь есть ещё и execution-aware слой:
    ...
    - тот же `chat_start_restore.prompt_text` теперь поднимает `Незавершённые линии к возврату`
      и явный `ExecCtl` warning, если в проекте уже есть suspended workline, чтобы новая рабочая поверхность
...
- live top-cards теперь честно отделяют `нужно внимание` от `ещё мало доказательной базы`:
  - `Скорость ответа`
    ...
    - это именно live-срез `current_session`, а не историческая карточка;
    - при новом live `session/window` выборка для неё начинается заново;
    - отдельный historical contour для этого блока пока не materialized и не должен подразумеваться UI по умолчанию;
    - если runtime пока дал только `mixed` live-срез без честного разделения на `hot/cold`, карточка
      обязана показывать общий live поток этой сессии, а не пустые `ещё нет данных`;
    - в таком режиме из карточки убираются искусственные `hot/cold`-строки и остаётся только то,
      что действительно известно сейчас: общий `P50/P95/P99/Max`, последний запрос и размер live-выборки;
  - `Текущая работа`
...
- machine-cards теперь показывают не только статическую сводку, но и живую hardware-телеметрию:
  ...
  - при этом статическая host-инвентаризация памяти больше не должна заново ходить в `dmidecode`
    на каждом минутном TTL machine summary:
    - memory type / speed теперь переиспользуются как отдельный static-host cache до `6` часов;
    - live machine cards продолжают обновлять usage-поля, но не крутят privileged memory probe
      раз в минуту без реального смысла.
...
Если `Amai` стоит стандартным GitHub install-path в `~/.local/share/amai/repo`, эта одна команда теперь делает полный uninstall:
- снимает client config;
- останавливает и убирает `amai-stack.service`;
- опускает local stack;
- удаляет runtime tree и managed clone.
...
Если нужно снять только client integration, но оставить сам локальный install/runtime, используйте:
...
- в human dashboard теперь есть отдельная карточка `Локальный мусор и retention`, где явно видны:
  ...
  - какие exact operator reclaim commands сейчас являются ближайшими кандидатами для ручного возврата места;
...
- если при этом на основном диске уже crossed pressure-thresholds, тот же policy-held хвост обязан эскалироваться из `waiting` в actionable `alert/critical` с ручной подсказкой на target-specific aggressive reclaim path;
- даже в обычном `waiting` карточка и warning теперь должны показывать ближайший exact reclaim path для самого тяжёлого retained target, а не только абстрактно говорить про TTL/keep-latest;
...
- этот блок описывает materialized eval semantics и снова подкреплён свежим green proof bundle;
- `./scripts/proof_benchmark_contamination_preflight.sh` зелёный и покрывает strict-heavy fail-closed path для latency-sensitive benchmark-ов;
- `./scripts/proof_mcp_task_matrix.sh` зелёный после strict-heavy contamination preflight и explicit failed-run artifact handling;
- dependent `./scripts/proof_observability.sh` зелёный; dashboard `MCP task matrix compare` больше не деградирует в `ещё нет данных` после MCP matrix proof path.
